...
class DiseasePagingSource(private val apiService: ApiService, private val token: String) : PagingSource<Int, Disease>(){
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Disease> {
        return try {
            val position = params.key ?: INITIAL_PAGE_INDEX
            val responseData = apiService.getDiseases("Bearer $token", position, params.loadSize).toListDisease()
