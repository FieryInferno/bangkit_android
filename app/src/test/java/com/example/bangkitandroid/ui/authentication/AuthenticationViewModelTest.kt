...
class AuthenticationViewModelTest {
    ...
    fun `when Login Should Return Correct Data`() = runTest {
        val expectedLogin = DummyData().getLoginResult()
    ...
    fun `when Register Success Should Return Success`() = runTest {
        val expectedResponse = DummyData().getRegisterResult()

        val authViewModel = AuthenticationViewModel(repository)
        val actualResponse = authViewModel.register("dummy".toRequestBody(), "000000".toRequestBody(), "password".toRequestBody(), null)
